...
|----|------|--------|-------|
| F1-001 | Define canonical event types in `packages/event-schema` | Not started | — |
| F1-002 | Create Zod validators for all event types | Not started | — |
...
| F1-007 | Unit tests for event-schema validators | Not started | — |
...
- [ ] All canonical event types are defined with TypeScript interfaces
- [ ] Zod schemas validate all event types correctly
...
- [ ] > 80% test coverage on event-schema and replay-engine packages

---

### Notes

- Start with `event-schema` — everything else depends on it
